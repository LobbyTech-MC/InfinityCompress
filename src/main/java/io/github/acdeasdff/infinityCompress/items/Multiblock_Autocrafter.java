package io.github.acdeasdff.infinityCompress.items;
...
//io.ncbpfluffybear.FluffyMachines
public class Multiblock_Autocrafter extends SlimefunItem implements EnergyNetComponent {
    ...
    public Multiblock_Autocrafter(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, String displayName, Material material, String machineName, RecipeType machineRecipes) {
        ...
        new BlockMenuPreset(getId(), displayName) {
            ...
            public void newInstance(@Nonnull BlockMenu menu, @Nonnull Block b) {
                if (!BlockStorage.hasBlockInfo(b)
                    ...
                    menu.replaceExistingItem(6, new CustomItemStack(Material.GUNPOWDER, "&7状态: &4\u2718", "",
                            "&e> 点击开启机器")
                    );
                    ...
                    menu.replaceExistingItem(6, new CustomItemStack(Material.REDSTONE, "&7状态: &2\u2714",
                            "", "&e> 点击关闭机器")
                    );
                    menu.addMenuClickHandler(6, (p, slot, item, action) -> {
    ...
    public void preRegister() {
        addItemHandler(new BlockTicker() {
            ...
            public void tick(Block b, SlimefunItem sf, Config data) {
                Multiblock_Autocrafter.this.tick(b);
            }
    ...
    private boolean isCraftable(BlockMenu inv, ItemStack[] recipe) {
        ...
        for (int j = 0; j < 9; j++) {
            ...
            if (recipe[j] == null){
                recipeItemAmount = 0;
            }else {recipeItemAmount = recipe[j].getAmount();}
            if (
                    (item != null && item.getAmount() <= recipeItemAmount && item.getType().getMaxStackSize() != 1)
    ...
    private void craft(ItemStack output, BlockMenu inv, ItemStack[] recipe) {
        for (int j = 0; j < 9; j++) {
            ItemStack item = inv.getItemInSlot(getInputSlots()[j]);

            if (item != null && item.getType() != Material.AIR) {
                inv.consumeItem(getInputSlots()[j], recipe[j].getAmount());
            }
        }
